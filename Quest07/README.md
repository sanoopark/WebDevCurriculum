...
- 이번 퀘스트에서는 node.js의 기본적인 구조와 개념에 대해 알아보겠습니다.
...
- Node.js : V8 엔진에 비동기 처리 라이브러리인 Libuv를 결합한 자바스크립트 런타임 환경
...
- Node.js의 기본 패키지 관리자
- 패키지 : 코드의 배포를 위해서 사용되는 코드의 묶음 (cf. 라이브러리 : 코드 작성을 위해 필요한 코드의 묶음)
- 패키지 관리자 : 패키지를 관리하는 작업을 자동화, 안전 처리 하기 위해 사용되는 도구
...
- `package.json` : 확장 모듈을 배포할 때 해당 모듈 정보를 담거나, 애플리케이션을 개발할 때 확장 모듈에 대한 의존성을 관리하기 위해 사용하는 파일
  ...
  - `version`
    ...
    - 프로젝트 버전을 정의합니다.
    - 3단계 버전을 사용하며, - 로 태그 이름을 적을 수 있습니다.
  ...
  - `author`
    ...
    - JSON 형식으로 name, email, url 옵션을 포함합니다.
  ...
  - `repository`
    ...
    - 소스 코드에 참여하고자 하는 사람들에게 알려줍니다.
    - 프로젝트의 홈페이지 url을 명시해서는 안 됩니다.

  - `scripts`

    - 프로젝트에서 자주 실행해야 하는 명령어를 작성해두면 npm 명령어로 실행 가능합니다.
...
- `-g` 글로벌 옵션
  ...
  - 해당 프로젝트 뿐만 아니라 다른 프로젝트에서도 해당 모듈을 사용할 수 있게 시스템 디렉토리에 설치하는 옵션
...
- ESM(ECMAScript Module)
  ...
  - `import`, `export`
  - 표준 자바스크립트 모듈 시스템
  - CJS 문법, AMD 비동기 로드 같은 이전 모듈 시스템들의 장점들을 채택했습니다.
  - 최신 브라우저에서도 모듈 시스템을 사용할 수 있습니다.
...
- ES Moudles가 등장한 이유
  ...
  - CJS 방식은 비동기 로드가 아니므로 브라우저 호환과 표준화에 대한 요구로 ES Modules가 등장했습니다.
